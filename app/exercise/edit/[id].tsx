...
export default function EditExerciseScreen() {
    ...
    const { data: exerciseData, isLoading: isLoadingEx } = useExercise(Number(id));
    ...
    useEffect(() => {
        if (exerciseData) {
            ...
            if (data) {
                ...
                setCategoryId(data.category || data.category_detail?.id || null);
    ...
    const loading = isLoadingCats || isLoadingEx;
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (isGuest) {
                const updatedEx = {
                    ...exerciseData, // Keep old properties (especially id/exercise_id)
                    name,
                    description,
                    category: categoryId,
                ...
                queryClient.setQueryData(
                    ...
                    (oldData: any) => {
                        ...
                        return data.map(ex => (ex.exercise_id || ex.id) === Number(id) ? updatedEx : ex);
                    }
                );

                queryClient.setQueryData(EXERCISE_KEYS.detail(Number(id)), updatedEx);
            ...
            if ((error.isAxiosError || error.name === 'AxiosError' || error.message === 'Network Error') && !error.response) {
                Alert.alert('Keine Verbindung', 'Übung konnte nicht aktualisiert werden, da keine Verbindung besteht.');
                return;
            }

            let errorMessage = 'Übung konnte nicht aktualisiert werden.';
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor }]} edges={['bottom']}>
            <ScrollView contentContainerStyle={styles.content}>
                <ThemedText type="title" style={styles.header}>Übung bearbeiten</ThemedText>
                ...
                >
                    {submitting ? (
                        ...
                        <ThemedText style={styles.submitButtonText}>Speichern</ThemedText>
                    )}
                </TouchableOpacity>
